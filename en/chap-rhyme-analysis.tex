...
Another difference is that Plechac only looks at the last word and 1 to 2 syllables while we at everything after the last stress up to 4 syllables.

For the matrix initialization, Plechac calculates the collocations in the data and preserves only pairs with number of collocations above a set threshold. We instead initialized it with default value of 0.2 as we believe that to be sufficient. 
...
\subsection{Rhyme rating}
 ...
 For each rhyme, we would like to assign a rating between 0 and 1. Since perfect rhyme is often in literature described as superior as it represent the perfect match in sound it will logically receive the highest rating of 1.
