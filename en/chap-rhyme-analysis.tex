...
Then we progress through individual steps needed to find the rhymes and design a rating that would \added{evaluate} song's rhymes:
...
\begin{enumerate}
	...
	\item finding all rhymes and assigning \added{a} scheme
...
\label{sparsar_wrong_scheme}
\end{table}
\MP{Dovolil jsem si do tabulky \ref{sparsar_wrong_scheme} přidat sloupec s correct scheme.
Možná by šlo ještě do caption přidat něco jako
 The correct scheme is marked in capital letters,
 the scheme assigned by SPARSAR is marked in lowercase letters.
Případně obojí převeďte na stejnou velikost, ale jde o to naznačit,
 že schémata nelze porovnávat na přesnou shodu.
Pokud nesouhlasíte, původní verze tabulky je zakomentovaná ve zdrojáku.
Mimochodem: proč SPARSAR přiřadil druhé řádce zrovna ``h''?
Nebylo to kvůli nějakým předchozím veršům té básně?
A kdyby se daly SPARSARu pouze tyto 4 verše, tak tomu přiřadí abac?
Pokud ano, tak by asi bylo lepší tam napsat toto schéma,
 ať si s tím h čtenáři nelámou hlavu jako teď já.}
...
CMUdict is a large dictionary and it includes also slang words so it should cover most of our input. To test this, we looked at all last words on each line of our data (since those are the important ones for rhyme analysis) and we found out that 5.52\% of them are not included in CMU dictionary. We manually inspected a small portion of them and found out that they can be mostly split into \replaced{the following}{these} 5 categories:
...
In an attempt to decrease the amount of out-of-dictionary words, we replaced the closing \added{single} quotation mark ``\textipa{’}'' (U+2019) with the typewriter apostrophe ``\texttt{\fontencoding{OT1}\selectfont\symbol{13}}''
...
\MP{Šlo by též zmínit, že \textit{syllabify} potřebuje na vstupu fonetický přepis v ARPAbet,
 k čemuž používáte výstup z CMUdict, tedy nemůže nastat, že by ty dva nástroje vrátily různou výslovnost.}


\subsection{Extracting relevant components}\label{extracting_relevant_phonemes}
Since we established that rhymes are located at the end of each line, there is no need to analyze the entire verse. How far should we look? The first choice would be to look at the last word \added{only}. However rhymes can extend over more words as we see in example in Figure \ref{two-word_rhyme}.
...
To set ourselves apart from simple rhyme detectors, we need to detect more than just perfect and imperfect rhymes -- we need to find forced rhymes as well. To determine forced rhyme\added{s} we have to assess the match in sound of individual phonemes.

For this, we took inspiration from \cite{plechavc2018collocation}. He used the fact that rhymes tend to reoccur and, having enough data, commonly co-occurring pairs are most probably rhymes even without the knowledge of their pronunciation. He calculated the probabilities of phoneme co-occurrences in line-end words from their frequencies in data, used this \added{co-occurrence} matrix to predict rhymes, and then iteratively recalculated the matrix using \added{the} EM algorithm.

However, our system of alignment is different so our matrix components will look differently. The differences are shown in Table \ref{alignment}. The first difference is that Plecháč only \replaced{uses}{has} vowels and consonant clusters without acknowledging the syllable separation. We do separate the consonant cluster into two on the border of syllable\added{s} (e.g. ``rp`` in carpenter is in one cluster for Plecháč, but \added{in two} separate clusters for us). The second difference is that Plecháč recognized the position of the component and paired only the components on the same position (i.e. both ``\textipa{@}'' in carpenter are in separate groups because they occur on different positions in the word). We do not believe that position has an effect on phoneme similarity and therefore we add it together for all positions, e.g. if once two phonemes co-occur on 3\textsuperscript{rd} position and once on the 5\textsuperscript{th} we will say that this pair of phonemes has 2 co-occurrences.
...
Knowing that perfect match in sound always means perfect rhyme \added{(which should intuitivelly get the highest possible rhyme rating of 1.0)}, we froze the values on matrix diagonal to reflect it. In \cite{plechavc2018collocation}, the diagonal was being recalculated as the rest of the matrix, and although it seemed to converge to high numbers we felt it did not reflect the superiority of the perfect match.

To calculate the probabilities in the matrix, we used the formula from \cite{plechavc2018collocation} adding the adjustments described above. The formula for a the value in the matrix on coordinates i, j is \replaced[comment={asi by šlo i the following}]{as follows}{following}: 
...
\MP{Collocation je normálně nějaká fráze -- slova těsně za sebou.
Buď důsledně všude pište vertical collocation, nebo na začátku upozorněte čtenáře,
že pro jednoduchost budete místo vertical collocation psát jen collocation.}

For the matrix initialization, Plecháč calculates the collocations in the data and preserves only pairs with number of collocations above a set threshold. We instead initialized it with \added[comment={nebo neurčitý člen a, nejsem si jist}]{the} default value of 0.2. 

Finding rhymes (step 2) is further described in Section \ref{finding-all-rhymes}.


\subsection{Rhyme rating}\label{rhyme-rating}
 We will first calculate rhyme ratings for pairs of verses and then use them to calculate an overall song rating. Not only do these rhyme ratings help us evaluate the rhyming quality of the song but they might also be an interesting feedback for the \replaced{users (e.g. authors of song lyrics) -- we thus show the rhyme ratings in our web visualization as described in Section~\ref{visualization-matrix}}{writer}.
...
\MP{Aha, já jsem někde výše navrhoval ``decreased by stress penalty'', protože jsem myslel, že stress\_penalty=0.1. Ono by to šlo tak i tak, jen by to mělo být konzistentní, tak buď změňte můj návrh, nebo tento vzoreček.}
\replaced{Indices}{Indexes} \textit{i, j} iterate over components in rhyming parts of the both words.
